...
all.cancers <- foreach(ix.cancer=1:length(infiles)) %dopar% {
## for (ix.cancer in 1:length(infiles)) {
    ...
    BB <- 9999
    ## BB <- 9
    seed.base <- 20200202 + ix.cancer*1e5
    bootstrap.out <- list()
    for (b in 1:BB) {
        this.seed <- seed.base+b
        set.seed(this.seed)
        if ((b-1) %% 10 == 0) {
            cat(sprintf("%s - % 6s % 5d\n", Sys.time(), cancer, b))
        }
        ...
        y.b <- rep(1, N)
        y.b.tab <- y.b %>% table %>% c
        while ((length(y.b.tab) == 1) | (!all(y.b.tab>10))) {
            bootstrap.ix <- sample(N, replace=TRUE)
            y.b <- y[bootstrap.ix]
            y.b.tab <- y.b %>% table %>% c
            k <- k+1
        }
        ...
        AA.miEMC <- estimate_adj(A.cancer, x.b, y.b)
        subset.miEMC <- list(AA=AA.miEMC,
                             x=x.b)

        make.subset <- function(regex, negate=TRUE, x, y) {
        ...
        subset.EMC <- make.subset("(^MIR|^LET|-mi$)", x=x.b, y=y.b)
        subset.E <- make.subset("(^MIR|^LET|-mi$|-CN$|-ME$)", x=x.b, y=y.b)
        subset.miE <- make.subset("(^MIR|^LET|-GE$)", negate=FALSE, x=x.b, y=y.b)
        ...
        for (i in 1:length(subsets)) {
            ...
            gsa <- NetGSA(AA, xx, y.b, B=B, directed=TRUE, lklMethod="REML")
        ...
        bootstrap.out[[b]] <- list(bootstrap.ix=bootstrap.ix,
                                   gsa=gsa,
                                   est=est,
                                   seed=this.seed)
    }
